...
/// Returns `true` if the endpoint is reachable (HTTP 200 or 401 — auth needed but live).
async fn probe_endpoint(http: &reqwest::Client, base_url: &str) -> bool {
    let url = format!("{base_url}/v1/models");
    match http.get(&url).send().await {
        Ok(resp) => {
            let s = resp.status().as_u16();
            // 200 = healthy, 401 = auth required but endpoint is live, 403 = same
            s == 200 || s == 401 || s == 403
        }
        Err(_) => false,
    }
...
///
/// `endpoints` — list of `(registry_key, base_url)` pairs where `registry_key`
/// is the `"provider:model"` string used in `EngineRegistry::by_name` and the
/// SmartRouter's `engine_health` map.
///
/// `interval_secs` — how often to probe. Defaults to 120 s (2 min).
